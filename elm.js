...
var $author$project$Main$base64 = A2($elm$core$Basics$composeR, $ivadzy$bbase64$Base64$Encode$bytes, $ivadzy$bbase64$Base64$Encode$encode);
...
var $author$project$Main$ChangeTimeZone = function (a) {
	return {$: 'ChangeTimeZone', a: a};
};
...
var $author$project$Main$getTimeZone = A2($elm$core$Task$perform, $author$project$Main$ChangeTimeZone, $elm$time$Time$here);
...
var $author$project$Main$init = F3(
	function (mflags, url, key) {
		...
		var model = _v1.a;
		var urlCmd = _v1.b;
		var _v2 = $truqu$elm_oauth2$OAuth$Implicit$parseToken(url);
		switch (_v2.$) {
			case 'Empty':
				return _Utils_Tuple2(
					_Utils_update(
					...
					$author$project$Main$getTimeZone);
			case 'Success':
				var token = _v2.a.token;
				var state = _v2.a.state;
				if (mflags.$ === 'Nothing') {
					return _Utils_Tuple2(
						_Utils_update(
							model,
					...
					return (!_Utils_eq(
						...
						_Utils_update(
							model,
							{
								flow: $author$project$Main$Authorized(token),
								redirectUri: redirectUri
							}),
						$elm$core$Platform$Cmd$batch(
							_List_fromArray(
								[
									$author$project$Main$getTimeZone,
									A3($andrewMacmurray$elm_delay$Delay$after, 350, $andrewMacmurray$elm_delay$Delay$Millisecond, $author$project$Main$UserInfoRequested),
									clearUrl
								])));
				}
			default:
...
var $author$project$Main$GotETA = function (a) {
	return {$: 'GotETA', a: a};
};
...
var $author$project$Main$arrivedETA = _Platform_incomingPort('arrivedETA', $elm$json$Json$Decode$value);
var $elm$core$Platform$Sub$batch = _Platform_batch;
var $elm$json$Json$Decode$decodeValue = _Json_run;
var $author$project$Main$ETA = F5(
	function (what, when, createdAt, userName, userPicture) {
		return {createdAt: createdAt, userName: userName, userPicture: userPicture, what: what, when: when};
	});
...
var $author$project$Main$etaDecoder = A6(
	$elm$json$Json$Decode$map5,
	$author$project$Main$ETA,
	A2(
	...
	return $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				$author$project$Main$arrivedETA(
				A2(
					$elm$core$Basics$composeR,
					$elm$json$Json$Decode$decodeValue($author$project$Main$etaDecoder),
					$author$project$Main$GotETA)),
...
var $author$project$Main$buildETA = F4(
	function (model, user, what, when) {
		return {
			createdAt: $elm$time$Time$posixToMillis(model.currentTime),
			userName: user.name,
...
var $author$project$Main$convertWhen = F2(
	function (model, when) {
		var s = A2(
			$elm$core$String$split,
			':',
			$elm$core$String$trim(when));
		var _v0 = _Utils_Tuple2(
			$elm$core$List$head(s),
			$elm$core$List$tail(s));
		var first = _v0.a;
		var rest = _v0.b;
		var _v1 = _Utils_Tuple2(first, rest);
		if ((_v1.a.$ === 'Just') && (_v1.b.$ === 'Just')) {
			var firstValue = _v1.a.a;
			var restValue = _v1.b.a;
			var _v2 = $elm$core$List$head(restValue);
			if (_v2.$ === 'Just') {
				var secondValue = _v2.a;
				var _v3 = _Utils_Tuple2(
					$elm$core$String$toInt(firstValue),
					$elm$core$String$toInt(secondValue));
				var hoursMaybeInt = _v3.a;
				var minutesMaybeInt = _v3.b;
				var _v4 = _Utils_Tuple2(hoursMaybeInt, minutesMaybeInt);
				if ((_v4.a.$ === 'Just') && (_v4.b.$ === 'Just')) {
					var hoursInt = _v4.a.a;
					var minutesInt = _v4.b.a;
					var currentParts = A2($justinmimbs$time_extra$Time$Extra$posixToParts, model.timeZone, model.currentTime);
					return $elm$core$Maybe$Just(
						$elm$time$Time$posixToMillis(
							A2(
								...
								_Utils_update(
									currentParts,
									{hour: hoursInt, millisecond: 0, minute: minutesInt, second: 0}))));
				} else {
					return $elm$core$Maybe$Nothing;
				}
			} else {
...
var $author$project$Main$etaEncoder = function (eta) {
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'what',
				$elm$json$Json$Encode$string(eta.what)),
				_Utils_Tuple2(
				'when',
				$elm$json$Json$Encode$int(eta.when)),
				_Utils_Tuple2(
				'created_at',
				$elm$json$Json$Encode$int(eta.createdAt)),
				_Utils_Tuple2(
				'user',
				$elm$json$Json$Encode$object(
					_List_fromArray(
						[
							_Utils_Tuple2(
							'name',
							$elm$json$Json$Encode$string(eta.userName)),
							_Utils_Tuple2(
							'picture',
							$elm$json$Json$Encode$string(eta.userPicture))
						])))
			]));
};
var $elm$browser$Browser$Dom$focus = _Browser_call('focus');
var $author$project$Main$pushNewETA = _Platform_outgoingPort('pushNewETA', $elm$core$Basics$identity);
var $author$project$Main$addETA = F2(
	function (model, user) {
		if ($elm$core$String$isEmpty(model.whatField)) {
			return _Utils_Tuple2(
				model,
				A2(
					...
					$elm$browser$Browser$Dom$focus('what-field')));
		} else {
			if ($elm$core$String$isEmpty(model.whenField)) {
				return _Utils_Tuple2(
					model,
					A2(
						$elm$core$Task$attempt,
						...
						$elm$browser$Browser$Dom$focus('when-field')));
			} else {
				var when = A2($author$project$Main$convertWhen, model, model.whenField);
				if (when.$ === 'Just') {
					var whenMilliseconds = when.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{whatField: '', whenField: ''}),
						$elm$core$Platform$Cmd$batch(
							_List_fromArray(
								[
									...
									$elm$browser$Browser$Dom$focus('what-field')),
									$author$project$Main$pushNewETA(
									$author$project$Main$etaEncoder(
										A4($author$project$Main$buildETA, model, user, model.whatField, whenMilliseconds)))
								])));
				} else {
...
var $author$project$Main$gotEta = F2(
	function (model, eta) {
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					etas: A2($elm$core$List$cons, eta, model.etas)
				}),
			$elm$core$Platform$Cmd$none);
	});
...
var $author$project$Main$configuration = {
	authorizationEndpoint: _Utils_update(
		...
		{host: 'jardev.auth0.com', path: '/authorize'}),
	clientId: 'B24wma3KZIYZojn2ANrMzsOAqIOzwVD6',
	...
	userInfoEndpoint: _Utils_update(
		$author$project$Main$defaultHttpsUrl,
		{host: 'jardev.auth0.com', path: '/userinfo'})
};
...
var $truqu$elm_oauth2$OAuth$Implicit$makeAuthorizationUrl = function (_v0) {
	...
	return A2(
		...
		$truqu$elm_oauth2$Internal$Token,
...
var $author$project$Main$loadETAs = _Platform_outgoingPort(
	'loadETAs',
	function ($) {
	...
	function (model, userInfoResponse) {
		if (userInfoResponse.$ === 'Err') {
			...
			return _Utils_Tuple2(
				...
				$author$project$Main$loadETAs(_Utils_Tuple0));
		}
	});
			...
			switch (_v0.b.$) {
				...
				case 'GotETA':
					if (_v0.a.$ === 'Done') {
						...
						if (res.$ === 'Ok') {
							var eta = res.a;
							return A2($author$project$Main$gotEta, model, eta);
						} else {
...
var $author$project$Main$displayCurrentTime = function (model) {
...
var $author$project$Main$AddETA = {$: 'AddETA'};
var $author$project$Main$UpdateWhatField = function (a) {
	return {$: 'UpdateWhatField', a: a};
};
var $author$project$Main$UpdateWhenField = function (a) {
	return {$: 'UpdateWhenField', a: a};
};
...
var $author$project$Main$inputETA = F2(
	function (what, when) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
					...
					A2(
						...
						$rundis$elm_bootstrap$Bootstrap$Form$InputGroup$config(
							$rundis$elm_bootstrap$Bootstrap$Form$InputGroup$text(
								_List_fromArray(
									[
										$rundis$elm_bootstrap$Bootstrap$Form$Input$placeholder('What'),
										$rundis$elm_bootstrap$Bootstrap$Form$Input$attrs(
										_List_fromArray(
											[
												$elm$html$Html$Events$onInput($author$project$Main$UpdateWhatField),
												$author$project$Main$onEnter($author$project$Main$AddETA),
												$elm$html$Html$Attributes$value(what),
												$elm$html$Html$Attributes$id('what-field')
											]))
									]))))),
					$rundis$elm_bootstrap$Bootstrap$Form$InputGroup$view(
					A2(
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$author$project$Main$onEnter($author$project$Main$AddETA),
												$elm$html$Html$Events$onClick($author$project$Main$AddETA)
											]))
									]),
								_List_fromArray(
									[
						...
						A2(
							...
							$rundis$elm_bootstrap$Bootstrap$Form$InputGroup$config(
								$rundis$elm_bootstrap$Bootstrap$Form$InputGroup$text(
									_List_fromArray(
										[
											$rundis$elm_bootstrap$Bootstrap$Form$Input$placeholder('HH:MM'),
											$rundis$elm_bootstrap$Bootstrap$Form$Input$attrs(
											_List_fromArray(
												[
													$elm$html$Html$Events$onInput($author$project$Main$UpdateWhenField),
													$author$project$Main$onEnter($author$project$Main$AddETA),
													$elm$html$Html$Attributes$value(when),
													$elm$html$Html$Attributes$id('when-field')
												]))
										]))))))
				]));
	});
...
var $author$project$Main$formatPosix = F2(
	function (tz, time) {
		return A3(
			$elm$core$String$padLeft,
			2,
...
var $author$project$Main$formatETA = F2(
	function (tz, eta) {
		return A2(
			$author$project$Main$formatPosix,
			tz,
			$elm$time$Time$millisToPosix(eta));
	});
...
var $author$project$Main$viewETA = F2(
	function (model, eta) {
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
								...
								[
									...
									_List_fromArray(
										[
											...
											$elm$html$Html$Attributes$src(eta.userPicture),
											A2($elm$html$Html$Attributes$style, 'margin-right', '5px')
										]),
									_List_Nil),
									$elm$html$Html$text(eta.userName)
								])),
							A2(
							$elm$html$Html$small,
							_List_Nil,
							_List_fromArray(
								[
									$elm$html$Html$text(
									A2($author$project$Main$formatETA, model.timeZone, eta.createdAt))
								]))
						])),
					A2(
					$elm$html$Html$h5,
					...
					_List_fromArray(
						[
							$elm$html$Html$text(
							eta.what + (' at ' + A2($author$project$Main$formatETA, model.timeZone, eta.when)))
						]))
				]));
	});
var $author$project$Main$viewETAs = F2(
	function (model, etas) {
		return $rundis$elm_bootstrap$Bootstrap$ListGroup$custom(
			A2(
				$elm$core$List$map,
				$author$project$Main$viewETA(model),
				etas));
	});
var $author$project$Main$pageDashboard = function (model) {
	return _List_fromArray(
		[
				...
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text('Expectations')
								])),
							...
							A3(
								...
								_List_fromArray(
									[
										...
										A3($elm$html$Html$Lazy$lazy2, $author$project$Main$viewETAs, model, model.etas))
									]),
								A3(
									$rundis$elm_bootstrap$Bootstrap$Card$block,
									_List_Nil,
									_List_fromArray(
										[
											$rundis$elm_bootstrap$Bootstrap$Card$Block$custom(
											A2($author$project$Main$inputETA, model.whatField, model.whenField))
										]),
									A3(
										...
										_List_fromArray(
											[
												$elm$html$Html$text(
												$author$project$Main$displayCurrentTime(model))
											]),
...
var $author$project$Main$pageHelp = _List_fromArray(
	[
		A2(
		$elm$html$Html$h1,
		_List_Nil,
		...
		$elm$html$Html$text('Here goes help page')
	]);
var $author$project$Main$pageNotFound = _List_fromArray(
	[
		...
		$elm$html$Html$text('The page is not found')
	]);
var $author$project$Main$mainContent = function (model) {
	return A2(
		...
		function () {
			...
			switch (_v0.$) {
				case 'Dashboard':
					return $author$project$Main$pageDashboard(model);
				case 'Help':
					return $author$project$Main$pageHelp;
				default:
					return $author$project$Main$pageNotFound;
			}
		}());
};
var $author$project$Main$SignOutRequsted = {$: 'SignOutRequsted'};
			...
			A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Welcome, ' + (userInfo.name + '!'))
							])),
						A2(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$class('rounded-circle'),
										$elm$html$Html$Attributes$src(userInfo.picture),
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$rundis$elm_bootstrap$Bootstrap$Utilities$Spacing$ml2Sm,
												$elm$html$Html$Events$onClick($author$project$Main$SignOutRequsted)
											]))
									]),
								_List_fromArray(
									[
...
var $author$project$Main$viewApp = F2(
	function (model, userInfo) {
		return A2(
			...
			_List_fromArray(
				[
					A2($author$project$Main$menu, model, userInfo),
...
var $author$project$Main$viewAuthorized = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
			$elm$html$Html$Attributes$class('d-flex min-vh-100 align-items-center justify-content-center')
		]),
	_List_fromArray(
		[
			A2(
	...
	return $elm$html$Html$text(
		function () {
			switch (e.$) {
				...
				default:
					return 'Unable to retrieve user info: HTTP request failed.';
			}
		}());
};
var $author$project$Main$viewErrored = function (error) {
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('span-error')
			]),
		_List_fromArray(
			[
										...
										A3(
											...
											_List_fromArray(
												[
													...
													_List_fromArray(
														[
															$elm$html$Html$text('In order to use ETA you have to sign in first')
														])),
													$rundis$elm_bootstrap$Bootstrap$Card$Block$custom(
													A2(
														$rundis$elm_bootstrap$Bootstrap$Button$button,
														...
														_List_fromArray(
															[
																$elm$html$Html$text('Sign In with OAuth0')
															])))
												]),
											A3(
												$rundis$elm_bootstrap$Bootstrap$Card$headerH4,
...
var $author$project$Main$view = function (model) {
	...
	switch (_v0.$) {
		...
		case 'Authorized':
			return $author$project$Main$viewAuthorized;
		case 'Done':
			var userInfo = _v0.a;
			return A2($author$project$Main$viewApp, model, userInfo);
		default:
			var err = _v0.a;
			return $author$project$Main$viewErrored(err);
	}
};
